...
public class BankCard implements Serializable {
	...
	/** cardholder -- 持卡人 */
	@Length(max=50,message="member.BankCard.cardholder.Length")
	private String cardholder;

	/** bankcardno -- 银行卡号 */
	@Length(max=50,message="member.BankCard.bankcardno.Length")
	private String bankcardno;

	/** collectingbank -- 收款银行 */
	@Length(max=50,message="member.BankCard.collectingbank.Length")
	private String collectingbank;

	/** reservedmobileno -- 银行预留手机号 */
	@Length(max=50,message="member.BankCard.reservedmobileno.Length")
	private String reservedmobileno;

	/** defaultcollectingbank -- 是否设为默认收款银行账户（0：否；1：是） */
	@NotNull(message="member.BankCard.defaultcollectingbank.NotNull")
	private Byte defaultcollectingbank;

	/** remark -- 备注 */
	@Length(max=30,message="member.BankCard.remark.Length")
	private String remark;

	/** defaultrefundbank -- 是否设为首选还款银行账户（0：否；1：是） */
	@NotNull(message="member.BankCard.defaultrefundbank.NotNull")
